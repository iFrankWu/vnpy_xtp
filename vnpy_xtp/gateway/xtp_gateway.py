...
class XtpTdApi(TdApi):
    ...
    def __init__(self, gateway: XtpGateway):
        ...
        self.retry_login_times = 0 # 每 3 秒一次，3600次，一共 3个小时
    ...
    def login_server(self) -> None:
        ...
        if n:
            ...
            if self.retry_login_times < self.max_retry_login_times:
                self.retry_login_times += 1
                self.gateway.write_log(f"登录失败，原因:{msg} 3秒后重试 已重试次数{self.retry_login_times} 最多不超过 {self.max_retry_login_times}")
                self.login_server()
